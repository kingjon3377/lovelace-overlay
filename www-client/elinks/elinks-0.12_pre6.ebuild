...
SRC_URI="http://elinks.or.cz/download/${MY_P}.tar.bz2
	...
	http://dev.gentoo.org/~axs/distfiles/${P/6/5}-js185-patches.tar.bz2"
	...
	if use javascript ; then
		if has_version ">=dev-lang/spidermonkey-1.8"; then
			if has_version ">=dev-lang/spidermonkey-1.8.5"; then
				epatch "${WORKDIR}"/patches/${P/6/5}-js185-1-heartbeat.patch
				epatch "${WORKDIR}"/patches/${P/6/5}-js185-2-up.patch
				epatch "${WORKDIR}"/patches/${P/6/5}-js185-3-histback.patch
